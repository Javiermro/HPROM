function [u,EPS_ROMI,eta,Q_ROMI,c_GdlCond,Fint,e_VarEst_new,e_VarAux,Du_step_new,c_CT,KT] = newton_raphson_MICRO(xx,...
   m_LinCond,dofl,doff,u,EPS_ROMI,eta,Q_ROMI,Du_step_new,c_GdlCond,Du_step_old,Fint,Fext,e_VarEst_old,e_VarAux,e_DatSet,...
   DefMacro,e_VG)
...
%sea cero.
if e_VG.MOD_TYPE==2
    e_VarEst_new = f_eVarEstInic({'sigma','eps','eps_fluct','hvar','delta_eps'},e_VG.ROM_II.e_DatSet_ROM,1,e_VG.MOD_TYPE);
%     e_VarEst_new = f_eVarEstInic({'sigma','eps','eps_fluct','hvar','delta_eps'},e_DatSet,1,e_VG.MOD_TYPE) ;
else
...
%
%ticIDEnsam = tic;
[KT,Fint,c_GdlCond,e_VarEst_new,e_VarAux] =f_Par_MatGlobales_MICRO(xx,u,Du_step_new,EPS_ROMI,eta,Q_ROMI,c_GdlCond,e_DatSet,...
...
if (e_VG.MOD_TYPE==1 || e_VG.MOD_TYPE==2)
    res = Fint;
else
    res = residuo_MICRO(Fext,Fint,m_LinCond,dofl,doff,lambda);
end
...
while ((norm_res > tol_rel || iter < 1) && (iter <= max_iter))
   ...
   elseif (e_VG.MOD_TYPE==2) % SECOND REDUCTION
       ...
       u = u + du_iter;   % DESPLAZAMIENTO TOTAL
%        Du_step_new = Du_step_new + du_iter;       
 ...
 %  c_GdlCond(:,1) = cellfun(@plus,c_GdlCond(:,1),c_dGdlCond,'UniformOutput',false);
   ...
   %ticIDEnsam = tic;
   [KT,Fint,c_GdlCond,e_VarEst_new,e_VarAux,c_CT] = f_Par_MatGlobales_MICRO(xx,u,Du_step_new,EPS_ROMI,eta,Q_ROMI,c_GdlCond,e_DatSet,...
      e_VarEst_new,e_VarEst_old,e_VarAux,DefMacro,e_VG);
   %fprintf('Tiempo de ensamblaje: %f\n',toc(ticIDEnsam));
...
%     if ~e_VG.esME
%       fprintf('ITERACION MICRO: %2d ; |res| = %-8.3e\n',iter,norm_res); %JLM
%     end
end
%fprintf('ITERACION: %2d ; |res| = %-8.3e\n',iter,norm_res);
% Weak problem (recovering displacements from strains)
% if (e_VG.MOD_TYPE==1 || e_VG.MOD_TYPE==2)
if ((e_VG.MOD_TYPE==1 || e_VG.MOD_TYPE==2) && e_VG.INV_PROB) % JLM
    if e_VG.MOD_TYPE==1
