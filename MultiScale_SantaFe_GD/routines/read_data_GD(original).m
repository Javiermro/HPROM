...
f_VerifNom(seccion,'END_FUNCTION',...
   'Función de evolución temporal: Se debe terminar la función con "END_FUNCTION".')
...
switch tipoCarga
   case 'POINT_LOAD'
      ...
      while ~strcmpi(c_ValVarCarga{1},'END_POINT_LOAD')
         ...
         %Con find(in==m_ValFza(1)) se convierte la numeración de nodos a la interna.
         m_gdlNod = f_DofElem(find(in==m_ValFza(1)),ndn);
         %Se realiza una suma de fuerzas, considerando que puede poner varias líneas con fuerzas en el
...
f_VerifNom(seccion,'END_LOAD',...
   'Cargas aplicadas: Se debe definir el final de la sección de ingreso de cargas, "END_LOAD".')
