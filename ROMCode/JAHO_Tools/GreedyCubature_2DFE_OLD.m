...
TIME_POINTS = tic ;
% dbstop('127')
while igaus <=ngausG  %& norm_resid>TOL
    maxTARG = -1e20 ;
...
if  PLOT_RESID == 1
    ...
    coloresMMMM =  ColoresMatrix(nmodes,[],'ALEATORIO','YES');
    MarkerMMMM =  MarkerMatrix(nmodes,[],'ALEATORIO','YES');
    hhh= [] ;
    hleg={} ;
    for imode=1:nmodes
        iini = (imode-1)*nmodesU + 1;
        ifin = imode*nmodesU ;
        error_loc = resid_VECTG(iini:ifin,:) ;
        norm_loc = sqrt(sum(error_loc.*error_loc));
        hhh(imode) =  plot(norm_loc,'Color',coloresMMMM(imode,:),'Marker',MarkerMMMM{imode}) ;
        hleg{imode} = ['mode=',num2str(imode)];
