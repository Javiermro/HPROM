...
%            case 2   %Plane stress � en el caso que se considere modificar tambi�n la Fzz del tensor.
%               J0 = Fzz*det(m_F2D);
%               pot = 3;
%            otherwise
%               error('Matrices Elementales FBar_q1: Tipo de c�lculo del FBar no implementado.')
%         end        
end
...
%     if ROM_ELEM_SET(iROM_GP)==3; p_COMP=p_bar2; end  % SOLO PARA RVE_Struct_Hole_01 JLM
   ...
   switch conshyp    
       case {100,110}
           % Gradiente de deformaci�n aplicado a cada punto de Gauss.   
           m_F = eps_new(:,iROM_GP);          
           %Matriz 2D de F
          ...
          switch tipoFBar
              case 1  %Plane deformation
                 %Se utiliza lo propuesto por Souza, de no modificar la componente Fzz del tensor.         
                 % Determinante del gradiente de deformaci�n.
                 J = det(m_F2D);
                 %Gradiente de deformaci�n FBar
%                  facFBar = (J0/J)^(1/pot);
                 facFBar = 1 ; % esto debe calcularse con J0, lo dejo asi a modo de prueba JLM
                 m_FBar([1;2;4;5]) = facFBar*m_F([1;2;4;5]);
                 %Por si acaso se mantiene el valor que componente zz, aunque deber�a ser 1.
                 m_FBar(3) = Fzz;
              case 2   %Plane stress � en el caso que se considere modificar tambi�n la Fzz del tensor.
                 % Determinante del gradiente de deformaci�n.
                 J = Fzz*det(m_F2D);            
...
%                  facFBar = (J0/J)^(1/pot);
    ...
    switch conshyp
      ...
      case 110 %Large deformations J2 Plasticity JLM
         ...
         %Tensor tangente el�stico de Henky en funci�n de G y K.
    ...
    if e_DatSet_ROM.e_DatMat.esImplex
        ...
        %tangentes implicitas para el an�lisis de bifurcacion.
        ...
        %Se almacena los tensores tangentes constitutivo implicitos para an�lisis de bifurcaci�n como si fuera
