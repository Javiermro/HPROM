...
% K_CONVEX_REG = zeros(e_VG_ME.nModesEPS_bar) ; JLM
...
%                         K_CONVEX_DIS = K_CONVEX_DIS + PHI_DIS(iPG_ID,:)'*e_Dat_iSet.m_VolElemGP(ipg,iElem)*PHI_DIS(iPG_ID,:);                         %JLM
%                         
                        
                    else
                        BmatRIT = PHI_REG(iPG_ID,:)'*e_Dat_iSet.m_VolElemGP(ipg,iElem) ;                        
                        if eltype==108
                            IntPhiTGen_REG = IntPhiTGen_REG + BmatRIT(:,[1 2 4 5]) ;% JLM
                        else
...
%                         K_CONVEX_REG = K_CONVEX_REG +PHI_REG(iPG_ID,:)'*e_Dat_iSet.m_VolElemGP(ipg,iElem)*PHI_REG(iPG_ID,:); JLM
...
% EnergySETS = 'DECOMP_01';
switch EnergySETS
    case 'NO_DECOMP'   % SIN DESCOMPOSICION DE DOMINIOS
        SetCOMP{1} = any(SET_ENER_COMP,2);   % BANDAS Y MATRIZ
        
    case 'DECOMP_01'
        %Set_BANDAS = 1;
        SetCOMP{1} = SET_ENER_COMP(:,Set_MATRIX); %any(SET_ENER_COMP(:,Set_BANDAS+1:nSet),2); % MATRIZ JLM
...
% ***************************************
disp('SVD of energy snapshot matrix')
if ~isempty(BASIS_DATA)
    ...
    if exist('PHI_ENER_REG')
    %try
        disp('Energy snapshot SVD matrix readed')
        %load(BASIS_DATA,'PHI_ENER_REG','Sing_Val_ELAS_REG','Sing_Val_INELAS_REG')
        DATA_CUBATURE.STRAIN_ENERGY_METHOD=1;
    else
        disp('Stress snapshot SVD matrix readed')
        %load(BASIS_DATA,'PHI_STRESS_REG','SingVal_ELAST','SingVal_INELAST')
        BasisS = PHI_STRESS_REG(:,1:NBASIS_ENER_PREV_REG);
        SingVal.SV_EPS=[SingValEPS_ELAST;SingValEPS_INELAST];
        SingVal.SV_STRESS=[SingVal_ELAST;SingVal_INELAST];
        %SingVal.SV_EPS=[ones(length(SingValEPS_ELAST),1);ones(length(SingValEPS_INELAST),1)];
        %SingVal.SV_STRESS=[ones(length(SingVal_ELAST),1);ones(length(SingVal_INELAST),1)];  

        DATA_CUBATURE.STRAIN_ENERGY_METHOD=0;
...
%             Sing_Val=ones(size(Sing_Val,1),1);
            SingVal.SV_ENERGY=[Sing_Val_ELAS_REG; Sing_Val_INELAS_REG]; %JLM
                        
            U = PHI_ENER_REG;
            U_TRUNC = U(:,1:NBASIS_ENER_PREV_REG);
            
%             U_TRUNC = bsxfun(@times,U_TRUNC,Sing_Val(1:size(U_TRUNC,2))');
            U_TRUNC = bsxfun(@times,U_TRUNC,SingVal.SV_ENERGY(1:size(U_TRUNC,2))');
...
%             Sing_Val=ones(size(Sing_Val,1),1);
            SingVal.SV_ENERGY=[Sing_Val_ELAS_DIS; Sing_Val_INELAS_DIS]; %JLM
            
            U = PHI_ENER_DIS;
            U_TRUNC = U(:,1:NBASIS_ENER_PREV_DIS);
            %U_TRUNC = PHI_ENER_DIS;
...
% 
%                     %FACT=1/norm(b(1:end-1));
%                     
%                     %b(1:end-1)=FACT*b(1:end-1); G(1:end-1,:)=FACT*G(1:end-1,:);                     
%                     %b=10000*b; 
%                     
%                     %G = [U_TRUNC';ones(1,length(ponder_factors{iSetCOMP}))] ;
%                     %b = [GLOB_U'; sum(ponder_factors{iSetCOMP})] ;
...
% %                     if iSetCOMP==2
% %                         nmodesU = NBASIS_ENER_PREV -10 + 1 ;
% %                     else
%                         
%                         nmodesU = NBASIS_ENER_PREV + 1 - TRU_POINT; %JLM
%                     %end
...
%                     %    length(ponder_factors{iSetCOMP}),nmodesU,BOUND_WEIGHTS_m_mg,DATA_CUBATURE,UNSTRESSED_POINTS,...
%                     %    ponder_factors{iSetCOMP},setPoints);
%                     
%                     [setPoints,weights] = IntegratManuelEnergy(U_TRUNC,ponder_factors{iSetCOMP},nmodesU);
%                     
%                     %[setPoints,weights,norm_resid,TIME_POINTS,setPointsALL,Jmin,bMIN] = GreedyCubature_2DFE(bMIN,Jmin,...
%                     %    length(ponder_factors{iSetCOMP}),nmodesU,nmodesU,BOUND_WEIGHTS_m_mg,DATA_CUBATURE,UNSTRESSED_POINTS,...
%                     %    ponder_factors{iSetCOMP},setPoints);                                        
%                     
...
%             
%             %save(NAME_SAVE_GREEDY,'npoints_energy_SET','numat_B_SET'); %Unusefull save
...
% end
    ...
    if isempty(SET_GPS{iSetCOMP})
        ...
        %G = [U_TRUNC'; ones(1,length(GaussWeights))] ;
        %b = [GLOB_U'; sum(GaussWeights)] ;
        %Jmin = G ;  bMIN = b ;

        nmodesU = NBASIS_ENER_PREV + 1 - TRU_POINT; %JLM
        %nmodesU = 220 ;

        %if exist([path_file '/POINTS/Cubature_points_mine_375.mat'])
        %    load([path_file '/POINTS/Cubature_points_mine_375.mat'])
        %else
        %[setPoints,weights,TIME_Jmin,TIME_POINTS,DATAGEN,wSTRESSES] = ...
        %    EmpiricalCubatureMAnuel(npointsINPUT)
        CubatureSettings;

        [setPoints,weights,~,~,wSTRESSES]=EmpiricalCubatureMAnuel...
            (npointsINPUT,ModoPHI_REG,BasisS,DATA_CUBATURE,NAME_BASIS,...
            ponder_factors{iSetCOMP},ntens,SingVal);   % agregar ModoPHI_DIS

        %[setPoints,weights] = IntegratManuelEnergy(U_TRUNC,ponder_factors,nmodesU);
        CUB_DATA = [setPoints weights] ;
        %end
...
% Reduced Cubature integration method    
Q_SET = CUB_DATA_GEN{iSetCOMP}(:,2) ;
Q_SET_TMP = repmat(Q_SET',ntens,1);
Q_SET2 = Q_SET_TMP(:) ;
%W2 = diag(Q_SET2) ;
W2 = sparse(1:length(Q_SET2),1:length(Q_SET2),Q_SET2);
B_II_sigma{iSetCOMP}= ModoPHI_REG(strain_comp{iSetCOMP},:)'*W2 ;
if exist('PHI_ENER_REG') % Energy Method
    WCELL = arrayfun(@(x)diag(Q_SET_TMP(:,x)),1:size(Q_SET_TMP,2),'UniformOutput',false);
    WMAT = cell2mat(WCELL);
else % Method of Residual Forces
    Q_SET_TMP = wSTRESSES;
    WCELL = arrayfun(@(x)Q_SET_TMP(x)*eye(ntens),1:length(wSTRESSES),'UniformOutput',false);
    WMAT = cell2mat(WCELL);
end

if (~e_VG.esME) Omega=e_VG.omegaMicro; else Omega=e_DatSet.e_DatMat.omegaMicro; end
Q{iSetCOMP} = WMAT./Omega;
...
% PRINT SELECTED INTEGRATION POINTS
...
%if exist([path_file '/POINTS/Cubature_points_mine_375.mat'])
%    load([path_file '/POINTS/Cubature_points_mine_375.mat'],'wSTRESSES')
...
for iSet=1:nSet
    ...
    [iD,~] = ismember(setElem_red(:,1),e_Dat_iSet.m_NumElem) ;
    ROM_ELEM_SET(iD) = iSet ;
    if e_Dat_iSet.e_DatElem.eltype==31 && (~isempty(iD))
        element_set=setElem_red(iD,1) ;
        AUX_ksb=[];
        for iKsb=1:sum(iD)
            index=find(e_Dat_iSet.m_NumElem==element_set(iKsb));
            AUX_ksb(iKsb,1)=e_DatSet_iSet.e_DatElem.ksb(index);
        end
        ROM_ksb(iD)=AUX_ksb ;
    end
end
...
if ~e_VG.esME % CASO MONOSESCALA
    SetRef = e_DatSet(1);
else % CASO MULTIESCALA
    SetRef = e_DatSet.e_DatMat.e_DatSet(1);
end

e_DatSet_ROM.sihvare = SetRef.sihvare/npg ;
e_DatSet_ROM.siavare = SetRef.siavare/npg ;
e_DatSet_ROM.sitvare = SetRef.sitvare/npg ;
e_DatSet_ROM.dofpe   = SetRef.e_DatElem.dofpe;

e_DatSet_ROM.siavarpg = SetRef.e_DatMat.siavarpg;
e_DatSet_ROM.sihvarpg = SetRef.e_DatMat.sihvarpg;

e_DatSet_ROM.e_DatElem.npg = npg ;
e_DatSet_ROM.e_DatElem.nVarHistElem = SetRef.e_DatElem.nVarHistElem ;
e_DatSet_ROM.e_DatElem.nVarAuxElem  = SetRef.e_DatElem.nVarAuxElem ;
e_DatSet_ROM.e_DatElem.eltype       = SetRef.e_DatElem.eltype ;

%e_DatSet_ROM.e_DatElem.pointersVAE.p_kSD = SetRef.e_DatElem.pointersVAE.p_kSD; % SOLO USADO EN FRACTURA
%e_DatSet_ROM.e_DatElem.ksb          = ROM_ksb;

e_DatSet_ROM.e_DatMat.conshyp  = SetRef.e_DatMat.conshyp ;
e_DatSet_ROM.e_DatMat.esImplex = SetRef.e_DatMat.esImplex ;
e_DatSet_ROM.e_DatMat.tit      = SetRef.e_DatMat.tit ;

ROM_II.e_DatSet_ROM = e_DatSet_ROM ;
% 
% 
% % % COMPONENTES DE TENSION SELECCIONADAS TOTALES
% % nDOF_ENER_SC_TOT=any(nDOFEnergy_SC_TOT_SET,2); JLM
% 
