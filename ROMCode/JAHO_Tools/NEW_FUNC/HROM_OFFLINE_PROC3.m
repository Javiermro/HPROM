...
%             Sing_Val=ones(size(Sing_Val,1),1);
            SingVal.SV_ENERGY=[Sing_Val_ELAS_REG; Sing_Val_INELAS_REG];
%             SingVal.SV_ENERGY=ones(size(SingVal.SV_ENERGY,1),1); % JLM idem original
            
            %tolX=10e-18;            
            
            U = PHI_ENER_REG;
            U_TRUNC = U(:,1:NBASIS_ENER_PREV_REG);
            %U_TRUNC = PHI_ENER_REG;
...
%             ELEM = sort(ElemREG) ;
            ...
            if isempty(SET_GPS{iSetCOMP})
                ...
                [setPoints,weights,~,~,wSTRESSES]=EmpiricalCubatureMAnuel...
                    ...
                    ponder_factors{iSetCOMP},ntens,SingVal);
...
%             Sing_Val=ones(size(Sing_Val,1),1);
            ...
            SingVal.SV_ENERGY=[Sing_Val_ELAS_DIS; Sing_Val_INELAS_DIS];
%             SingVal.SV_ENERGY=ones(size(SingVal.SV_ENERGY,1),1); % JLM idem original
            %tolX=10e-12;
...
%             ELEM = sort(ElemDIS) ;
            ...
            % Integral for cubature method
            %GLOB_U=sum(bsxfun(@times,U_TRUNC,ponder_factors),1);
...
W2 = sparse(1:length(Q_SET2),1:length(Q_SET2),Q_SET2);
    ...
    switch iSetCOMP
        case 1 % REGULAR (CONTINUOUS) DOMAIN
%             B_II_sigma{iSetCOMP}=ModoPHI_REG(strain_comp{iSetCOMP},:)'*W2 ; JLM
            B_II_sigma{iSetCOMP}= ModoPHI_REG(COMP_STR_REG(strain_comp{iSetCOMP}),:)'*W2 ;
        case 2 % SINGULAR (DISCONTINUOUS) DOMAIN
%             B_II_sigma{iSetCOMP}=ModoPHI_DIS(strain_comp{iSetCOMP},:)'*W2 ; JLM
            B_II_sigma{iSetCOMP}= ModoPHI_DIS(COMP_STR_DIS(strain_comp{iSetCOMP}),:)'*W2 ; 
    end
