...
for iSetCOMP=1:length(SetCOMP)
    ...
    switch iSetCOMP
        case 1 % REGULAR (CONTINUOUS) DOMAIN
            ...
            U_TRUNC{1} = U(:,1:nModeEnergELAS);
            U_TRUNC{2} = U(:,nModeEnergELAS+1:NBASIS_ENER_PREV_REG);            
            %U_TRUNC{1} = bsxfun(@times,U(:,1:nModeEnergELAS),Sing_Val(1:nModeEnergELAS)');
            %U_TRUNC{2} = bsxfun(@times,U(:,nModeEnergELAS+1:NBASIS_ENER_PREV_REG),Sing_Val(nModeEnergELAS+1:NBASIS_ENER_PREV_REG)');
            ...
            GLOB_U{1}=sum(bsxfun(@times,U_TRUNC{1},ponder_factors{iSetCOMP}),1) ;
            GLOB_U{2}=sum(bsxfun(@times,U_TRUNC{2},ponder_factors{iSetCOMP}),1) ;
        ...
        case 2 % REGULAR DOMAIN
            ...
            U_TRUNC{1} = U(:,1:nModeEnergELAS);
            U_TRUNC{2} = U(:,nModeEnergELAS+1:NBASIS_ENER_PREV_DIS);
            %U_TRUNC{1} = bsxfun(@times,U(:,1:nModeEnergELAS),Sing_Val(1:nModeEnergELAS)');
            %U_TRUNC{2} = bsxfun(@times,U(:,nModeEnergELAS+1:NBASIS_ENER_PREV_DIS),Sing_Val(nModeEnergELAS+1:NBASIS_ENER_PREV_DIS)');
            ...
            if isempty(SET_GPS{iSetCOMP})              
                ...
                if (interpolation_method == 1)
                    ...
                    for iMode=1:2
                        ...
                        %G = [U_TRUNC{iMode}'; ones(1,length(ponder_factors{iSetCOMP}))] ;
                        %b = [GLOB_U{iMode}'; sum(ponder_factors{iSetCOMP})] ;
                        ...
                        CUB_DATA = [CUB_DATA; setPoints' weights] ;
